...
const S18_VIDEO_BUCKET="start11-videos";
const S18_CLOUD_CHUNK_SIZE=45*1024*1024; // under Supabase Free's 50 MB per-file limit
function s18CloudSession(){try{return (typeof session!=="undefined"&&session)||JSON.parse(localStorage.getItem(typeof SESSION_KEY!=="undefined"?SESSION_KEY:"start11SupabaseSession")||"null")}catch{return null}}
function s18CloudUserId(){const ss=s18CloudSession();if(ss?.user?.id)return String(ss.user.id);try{const token=ss?.access_token;if(!token)return"";const b=token.split(".")[1].replace(/-/g,"+").replace(/_/g,"/");return String(JSON.parse(decodeURIComponent(escape(atob(b)))).sub||"")}catch{return""}}
function s18SafePathPart(v){return String(v||"x").replace(/[^a-zA-Z0-9._-]/g,"_").slice(0,120)||"x"}
function s18StorageBase(){return String(typeof SUPABASE_URL!=="undefined"?SUPABASE_URL:"").replace(/\/$/,"")+"/storage/v1"}
function s18StorageHeaders(contentType){const ss=s18CloudSession(),h={apikey:typeof SUPABASE_KEY!=="undefined"?SUPABASE_KEY:""};if(ss?.access_token)h.Authorization=`Bearer ${ss.access_token}`;if(contentType)h["Content-Type"]=contentType;return h}
async function s18StorageError(res){let msg=`Storage-fejl (${res.status})`;try{const x=await res.json();msg=x?.message||x?.error||msg}catch{}return new Error(msg)}
async function s18UploadObject(path,blob){const url=`${s18StorageBase()}/object/${encodeURIComponent(S18_VIDEO_BUCKET)}/${path.split("/").map(encodeURIComponent).join("/")}`;const r=await fetch(url,{method:"POST",headers:{...s18StorageHeaders("application/octet-stream"),"x-upsert":"false"},body:blob});if(!r.ok)throw await s18StorageError(r)}
async function s18DownloadObject(path){const url=`${s18StorageBase()}/object/authenticated/${encodeURIComponent(S18_VIDEO_BUCKET)}/${path.split("/").map(encodeURIComponent).join("/")}`;const r=await fetch(url,{headers:s18StorageHeaders()});if(!r.ok)throw await s18StorageError(r);return await r.blob()}
async function s18DeleteCloudPaths(paths){if(!paths?.length)return;const url=`${s18StorageBase()}/object/${encodeURIComponent(S18_VIDEO_BUCKET)}`;const r=await fetch(url,{method:"DELETE",headers:s18StorageHeaders("application/json"),body:JSON.stringify({prefixes:paths})});if(!r.ok)throw await s18StorageError(r)}
async function s18CloudVideoBlob(mv,onProgress){const parts=[];for(let i=0;i<(mv.cloudChunks||[]).length;i++){onProgress?.(i,mv.cloudChunks.length);parts.push(await s18DownloadObject(mv.cloudChunks[i]))}onProgress?.(mv.cloudChunks.length,mv.cloudChunks.length);return new Blob(parts,{type:mv.type||"video/mp4"})}
async function s18SetVideo(file){const p=s18Project();if(!p||!file)return;if(!String(file.type||"").startsWith("video/")){visNotification?.("Vælg en videofil.");return}const uid=s18CloudUserId();if(!uid){visNotification?.("Du skal være logget ind for at gemme kampvideoen på din konto.");return}const old=p.matchVideo,id=s13Id("matchvideo"),team=s18SafePathPart(typeof activeTeamId!=="undefined"?(activeTeamId||"no-team"):"no-team"),stamp=Date.now(),base=`${uid}/${team}/${s18SafePathPart(p.id)}/match-${stamp}`,paths=[];try{visNotification?.("Uploader kampvideo til din konto…");for(let start=0,n=0;start<file.size;start+=S18_CLOUD_CHUNK_SIZE,n++){const end=Math.min(file.size,start+S18_CLOUD_CHUNK_SIZE),path=`${base}/part-${String(n).padStart(4,"0")}.bin`;await s18UploadObject(path,file.slice(start,end));paths.push(path);visNotification?.(`Uploader kampvideo… ${Math.round(end/file.size*100)}%`)}try{await s15StoreBlob(id,file)}catch(e){console.warn("START11: lokal videokopi kunne ikke gemmes",e)}p.matchVideo={sourceType:"cloud-chunks",mediaId:id,name:file.name,type:file.type||"video/mp4",size:file.size,url:"",cloudBucket:S18_VIDEO_BUCKET,cloudChunks:paths,cloudChunkSize:S18_CLOUD_CHUNK_SIZE};s18Touch();if(old?.mediaId)try{await s15DeleteBlob(old.mediaId)}catch{}if(old?.cloudChunks?.length)try{await s18DeleteCloudPaths(old.cloudChunks)}catch(e){console.warn("START11: gammel cloud-video kunne ikke slettes",e)}visNotification?.("Kampvideoen er gemt på din konto.");s18Render(document.getElementById("s13content"))}catch(e){console.error(e);if(paths.length)try{await s18DeleteCloudPaths(paths)}catch{}visNotification?.(`Kunne ikke gemme kampvideoen i cloud: ${e.message||e}`)}}
function s18SetVideoLink(){const p=s18Project();if(!p)return;const url=prompt("Direkte videolink:","https://");if(!url)return;p.matchVideo={sourceType:"link",mediaId:"",name:"Ekstern kampvideo",type:"video",size:0,url:url.trim()};s18Touch();s18Render(document.getElementById("s13content"))}
