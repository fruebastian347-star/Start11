...
function s18IsVeoUrl(url){try{const u=new URL(String(url||"").trim());return /(^|\.)veo\.co$/i.test(u.hostname)&&u.pathname.includes("/matches/")}catch(e){return false}}
function s18CleanVeoUrl(url){try{const u=new URL(String(url||"").trim());u.search="";u.hash="";return u.toString()}catch(e){return String(url||"").trim()}}
function s18VeoUrlAt(url,sec){const clean=s18CleanVeoUrl(url);const t=Math.max(0,Math.floor(Number(sec)||0));return clean+(clean.includes("?")?"&":"?")+"t="+t}
function s18SetVideoLink(){const p=s18Project();if(!p)return;const url=prompt("Veo-link eller direkte videolink:","https://app.veo.co/matches/");if(!url)return;const value=url.trim();if(s18IsVeoUrl(value)){p.matchVideo={sourceType:"veo",mediaId:"",name:"Veo-kamp",type:"veo",size:0,url:s18CleanVeoUrl(value)};s18Touch();s18Render(document.getElementById("s13content"));return}p.matchVideo={sourceType:"link",mediaId:"",name:"Ekstern kampvideo",type:"video",size:0,url:value};s18Touch();s18Render(document.getElementById("s13content"))}
...
async function s18LoadVideo(){const p=s18Project(),stage=document.getElementById("s18Stage");if(!p||!stage)return;if(s18.objectUrl){URL.revokeObjectURL(s18.objectUrl);s18.objectUrl=""}if(!p.matchVideo){stage.innerHTML=`<div id="s18Drop" class="s18-drop"><div><strong>DROP HELE KAMPVIDEOEN HER</strong><div>MP4 / WebM / MOV · gemmes lokalt i browseren</div><div style="margin-top:8px">Eller brug VIDEO-LINK til en Veo-kamp.</div></div><input id="s18DropFile" type="file" accept="video/*" hidden></div>`;const d=document.getElementById("s18Drop"),f=document.getElementById("s18DropFile");d.onclick=()=>f.click();d.ondragover=e=>{e.preventDefault();d.classList.add("dragover")};d.ondragleave=()=>d.classList.remove("dragover");d.ondrop=e=>{e.preventDefault();d.classList.remove("dragover");if(e.dataTransfer.files[0])s18SetVideo(e.dataTransfer.files[0])};f.onchange=e=>e.target.files[0]&&s18SetVideo(e.target.files[0]);return}if(p.matchVideo.sourceType==="veo"){const c=s18Clip(),start=c?.startSec||0,veo=s18VeoUrlAt(p.matchVideo.url,start);stage.innerHTML=`<div class="s18-drop" style="cursor:default"><div><strong>VEO-KAMP TILKNYTTET</strong><div style="margin-top:8px">Veo-videoen ligger hos Veo og fylder derfor ikke i START11.</div><div style="margin-top:10px;display:flex;gap:7px;justify-content:center;flex-wrap:wrap"><button id="s18OpenVeo" class="s13btn primary">ÅBN VEO${c?` FRA ${s18fmt(c.startSec)}`:""}</button><button id="s18CopyVeo" class="s13btn">KOPIÉR LINK</button></div><div class="s13mut" style="margin-top:10px">Veo kan ikke styres som en lokal videofil inde i START11. Klippets IN/OUT og analyse gemmes stadig på projektet.</div></div></div>`;document.getElementById("s18OpenVeo").onclick=()=>window.open(veo,"_blank","noopener");document.getElementById("s18CopyVeo").onclick=async()=>{try{await navigator.clipboard.writeText(veo);visNotification?.("Veo-link kopieret.")}catch(e){prompt("Kopiér Veo-link:",veo)}};return}let src="";if(p.matchVideo.sourceType==="local"){const x=await s15GetBlob(p.matchVideo.mediaId);if(!x?.blob){stage.innerHTML=`<div class="s18-drop">Den lokale kampvideo findes ikke på denne enhed.</div>`;return}src=URL.createObjectURL(x.blob);s18.objectUrl=src}else src=p.matchVideo.url||"";stage.innerHTML=`<video id="s18Video" src="${s13Esc(src)}" preload="metadata" playsinline></video><canvas id="s18Canvas"></canvas>`;s18BindVideo()}
